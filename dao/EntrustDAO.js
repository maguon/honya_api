...
function getEntrustBase(params,callback) {
    var query = " select e.*,count(case when r.rel_status = 1 then c.id end) as car_count, " +
        " if(isnull(sum(case when r.rel_status = 1 then c.valuation end)),0,sum(case when r.rel_status = 1 then c.valuation end)) as valuation, " +
        " count(case when c.mso_status = 1 then c.id end) as not_mso_count, " +
        " if(isnull(sum(case when c.mso_status = 1 then c.valuation end)),0,sum(case when c.mso_status = 1 then c.valuation end)) as not_mso_valuation " +
        " from entrust_info e " +
